    ...
    try {
      ...
      if (!validateResult.valid) {
        setSaveError(createParsedApiError({
          ...
          rawMessage: 'Config validation failed. Please fix form errors.',
          category: 'http_error',
        }));
        setRetryAction('save');
        return {
      ...
      const warningText = updateResult.warnings?.length
        ...
        : '';
      setToast({ type: 'success', message: `Config updated${warningText}` });
      return { success: true };
    } catch (error: unknown) {
      if (error instanceof SystemConfigValidationError) {
        ...
        setSaveError(createParsedApiError({
          ...
          message: `${error.message}. Please reload the config first.`,
