...
export const useAgentChatStore = create<AgentChatState & AgentChatActions>((set, get) => ({
  ...
  startStream: async (payload, meta) => {
    ...
    const strategyName = meta?.strategyName ?? 'General';
    ...
    try {
      ...
      const processLine = (line: string) => {
        ...
        if (event.type === 'done') {
          ...
          if (doneEvent.success === false) {
            const parsedStreamError = getParsedApiError(
              doneEvent.error ||
                doneEvent.content ||
                'LLM call failed, please check your API Key configuration',
            );
        ...
        if (event.type === 'error') {
          throw getParsedApiError(event.message || 'Analysis error');
        }
