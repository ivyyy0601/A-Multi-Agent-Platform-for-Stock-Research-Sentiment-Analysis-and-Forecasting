...
export const HistoryList: React.FC<HistoryListProps> = ({
  ...
  const getOperationBadgeLabel = (advice?: string) => {
    ...
    if (normalized.includes('减仓') || normalized.toLowerCase().includes('reduce')) {
      return 'Reduce';
    }
    if (normalized.includes('卖') || normalized.toLowerCase().includes('sell')) {
      return 'Sell';
    }
    if (normalized.includes('观望') || normalized.includes('等待') || normalized.toLowerCase().includes('wait') || normalized.toLowerCase().includes('hold')) {
      return 'Watch';
    }
    if (normalized.includes('买') || normalized.includes('布局') || normalized.toLowerCase().includes('buy')) {
      return 'Buy';
    }
    return normalized.split(/[，。；、\s]/)[0] || 'Signal';
  };
          ...
          {items.length > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex-1 flex items-center gap-2 px-2 py-1 rounded-lg bg-white/5 border border-white/5">
                ...
                />
                <span className="text-[11px] text-muted-text select-none">Select All</span>
              </div>
              <Button
                variant="danger"
                size="sm"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 space-y-3">
            ...
            <div className="space-y-1">
              <p className="text-sm text-secondary-text">No history records yet</p>
              <p className="text-xs text-muted-text">Recent results will appear here after your first analysis.</p>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            ...
            {!hasMore && items.length > 0 && (
              <div className="text-center py-5">
                ...
                <span className="text-[10px] text-muted-text/50 uppercase tracking-[0.2em]">End of list</span>
              </div>
            )}
          </div>
        )}
