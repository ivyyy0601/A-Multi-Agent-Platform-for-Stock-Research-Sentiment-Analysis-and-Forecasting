...
class FreeNewsSearchProvider(BaseSearchProvider):
    """
    Free news provider — no API key required.
    Aggregates: Yahoo Finance RSS, Google News RSS, CNBC RSS, Nasdaq RSS, SEC EDGAR.
    Always available; used as automatic fallback when no paid search keys are configured.
    """

    def __init__(self):
        super().__init__(api_keys=["free"], name="FreeNews")
        from data_provider.free_news_clients import (
            YahooFinanceNewsClient, GoogleNewsClient,
            CNBCNewsClient, NasdaqNewsClient, SECEdgarClient,
            extract_ticker_from_query,
        )
        self._yahoo = YahooFinanceNewsClient()
        self._google = GoogleNewsClient()
        self._cnbc = CNBCNewsClient()
        self._nasdaq = NasdaqNewsClient()
        self._sec = SECEdgarClient()
        self._extract_ticker = extract_ticker_from_query
    ...
    def _do_search(self, query: str, api_key: str, max_results: int, days: int = 7) -> SearchResponse:
        ...
        for client, limit in [
            ...
            try:
                raw.extend(client.fetch(ticker, limit=limit))
            except Exception as e:
                logger.debug("[FreeNews] %s.fetch error: %s", client.__class__.__name__, e)

        # Deduplicate by URL, filter by date window
        seen_urls: set = set()
        results: List[SearchResult] = []
        for item in raw:
            url = item.get("url", "")
            ...
            try:
                pub_dt = datetime.fromisoformat(pub_str).replace(tzinfo=timezone.utc) \
                    if pub_str and not pub_str.endswith('+00:00') \
                    else datetime.fromisoformat(pub_str.replace('Z', '+00:00')) \
                    if pub_str else None
            except Exception:
                pub_dt = None
            ...
            results.append(SearchResult(
                title=item.get("title", ""),
                snippet=item.get("text", item.get("title", "")),
                url=url,
                source=item.get("source", "FreeNews"),
                published_date=pub_str[:10] if pub_str else None,
            ))
        ...
        logger.info("[FreeNews] %s: fetched %d results in %.1fs (ticker=%s, days=%d)",
                    query[:60], len(results), elapsed, ticker, days)
        return SearchResponse(
            query=query,
            results=results,
            provider="FreeNews",
...
class SearchService:
    ...
    def __init__(
        ...
        # 7. FreeNews — always enabled as final fallback (Yahoo/Google/CNBC/Nasdaq/SEC RSS)
        self._providers.append(FreeNewsSearchProvider())
        logger.info("免费新闻源已启用 (Yahoo Finance / Google News / CNBC / Nasdaq / SEC EDGAR)")
