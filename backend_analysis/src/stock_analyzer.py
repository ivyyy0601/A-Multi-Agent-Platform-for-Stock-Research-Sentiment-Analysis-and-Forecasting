...
class TrendStatus(Enum):
    """Trend status enum"""
    STRONG_BULL = "Strong Bullish"      # MA5 > MA10 > MA20, gap widening
    BULL = "Bullish Alignment"          # MA5 > MA10 > MA20
    WEAK_BULL = "Weak Bullish"          # MA5 > MA10, but MA10 < MA20
    CONSOLIDATION = "Consolidation"     # MAs tangled
    WEAK_BEAR = "Weak Bearish"          # MA5 < MA10, but MA10 > MA20
    BEAR = "Bearish Alignment"          # MA5 < MA10 < MA20
    STRONG_BEAR = "Strong Bearish"      # MA5 < MA10 < MA20, gap widening


class VolumeStatus(Enum):
    """Volume status enum"""
    HEAVY_VOLUME_UP = "High Volume Rally"       # Volume and price both rising
    HEAVY_VOLUME_DOWN = "High Volume Decline"   # High volume selloff
    SHRINK_VOLUME_UP = "Low Volume Rally"       # Rising without volume
    SHRINK_VOLUME_DOWN = "Low Volume Pullback"  # Low volume pullback (positive)
...
class BuySignal(Enum):
    ...
    HOLD = "Hold"                 # Currently holding, continue
...
class MACDStatus(Enum):
    ...
    CROSSING_UP = "Crossing Above Zero"             # DIF crossing above zero
    CROSSING_DOWN = "Crossing Below Zero"           # DIF crossing below zero
...
class StockTrendAnalyzer:
    ...
    def _analyze_volume(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if result.volume_ratio_5d >= self.VOLUME_HEAVY_RATIO:
            if price_change > 0:
                ...
                result.volume_trend = "High volume decline, watch for risk"
        elif result.volume_ratio_5d <= self.VOLUME_SHRINK_RATIO:
            if price_change > 0:
                result.volume_status = VolumeStatus.SHRINK_VOLUME_UP
                result.volume_trend = "Low volume rally, limited upward momentum"
            else:
                result.volume_status = VolumeStatus.SHRINK_VOLUME_DOWN
                result.volume_trend = "Low volume pullback, consolidation pattern (positive)"
        else:
    ...
    def _analyze_macd(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if is_golden_cross and curr_zero > 0:
            ...
            result.macd_signal = "⚡ DIF crossing above zero, trend strengthening"
            ...
            result.macd_signal = "⚠️ DIF crossing below zero, trend weakening"
    ...
    def _analyze_rsi(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if rsi_mid > self.RSI_OVERBOUGHT:
            ...
            result.rsi_signal = f"✅ RSI Strong ({rsi_mid:.1f}), sufficient bullish momentum"
        elif rsi_mid >= 40:
            result.rsi_status = RSIStatus.NEUTRAL
            result.rsi_signal = f"RSI Neutral ({rsi_mid:.1f}), range-bound consolidation"
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if result.trend_status in [TrendStatus.STRONG_BULL, TrendStatus.BULL]:
            reasons.append(f"✅ {result.trend_status.value}, trend-following long")
        ...
        if bias < 0:
            ...
            if bias > -3:
                score += 20
                reasons.append(f"✅ Price slightly below MA5 ({bias:.1f}%), ideal pullback entry")
            elif bias > -5:
                score += 16
                reasons.append(f"✅ Price pulling back to MA5 ({bias:.1f}%), watching support")
            else:
                score += 8
            ...
            score += 10
            reasons.append(
                f"⚡ Bias elevated in strong trend ({bias:.1f}%), light position tracking OK"
            )
        else:
            score += 4
            risks.append(
        ...
        if result.volume_status == VolumeStatus.SHRINK_VOLUME_DOWN:
            reasons.append("✅ Low volume pullback, institutional accumulation pattern")
        elif result.volume_status == VolumeStatus.HEAVY_VOLUME_DOWN:
            risks.append("⚠️ High volume decline, watch for risk")
