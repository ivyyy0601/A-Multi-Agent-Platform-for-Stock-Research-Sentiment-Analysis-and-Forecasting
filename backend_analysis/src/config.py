...
class Config:
    ...
    # 单能力源调用超时（秒）
    # 美股 yfinance 基本面（info/financials/cashflow/dividends）偶发较慢；
    # 默认提高超时，避免像 TSLA / AAPL 这类大票频繁因 800ms 超时而丢失基本面。
    fundamental_fetch_timeout_seconds: float = 4.0
    # 单能力失败重试次数（已包含首次）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            fundamental_stage_timeout_seconds=float(
                os.getenv('FUNDAMENTAL_STAGE_TIMEOUT_SECONDS', '6.0')
            ),
            fundamental_fetch_timeout_seconds=float(
                os.getenv('FUNDAMENTAL_FETCH_TIMEOUT_SECONDS', '4.0')
            ),
            fundamental_retry_max=int(os.getenv('FUNDAMENTAL_RETRY_MAX', '2')),
