...
def extract_fundamental_detail_fields(
    ...
    # Merge growth fields (revenue_yoy, net_profit_yoy, gross_margin) into financial_report
    # so the frontend can display them alongside financials.
    # growth.data is a separate block at the same level as earnings in fundamental_context.
    if isinstance(financial_report, dict):
        ...
        if isinstance(growth_data, dict):
            for key in ("revenue_yoy", "net_profit_yoy", "gross_margin"):
                if growth_data.get(key) is not None:
                    financial_report.setdefault(key, growth_data[key])
