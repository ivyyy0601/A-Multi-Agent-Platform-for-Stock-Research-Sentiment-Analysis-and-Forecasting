...
class TaskStatus(str, Enum):
    ...
    WAITING_FOR_FUNDAMENTALS = "waiting_for_fundamentals"  # 等待最新基础面
...
class AnalysisTaskQueue:
    ...
    def list_pending_tasks(self) -> List[TaskInfo]:
        ...
        with self._data_lock:
            return [
                ...
                if task.status in (TaskStatus.PENDING, TaskStatus.PROCESSING, TaskStatus.WAITING_FOR_FUNDAMENTALS)
            ]
    ...
    def cancel_task(self, task_id: str) -> Optional[TaskInfo]:
        ...
        processing 任务无法强制终止底层线程，只会解除占坑并忽略后续结果回写。
        """
        with self._data_lock:
            task = self._tasks.get(task_id)
            if task is None:
            ...
            cancelled_before_run = bool(future.cancel()) if future is not None else False

            task.status = TaskStatus.CANCELLED
            task.completed_at = datetime.now()
            task.error = None
            task.message = "任务已取消" if cancelled_before_run else "任务已标记取消"

            if self._analyzing_stocks.get(task.stock_code) == task_id:
                del self._analyzing_stocks[task.stock_code]

            snapshot = task.copy()

        self._broadcast_event("task_cancelled", snapshot.to_dict())
        logger.info("[TaskQueue] 任务已取消: %s (%s)", task_id, snapshot.stock_code)
        return snapshot
        ...
        try:
            ...
            def progress_callback(payload: Dict[str, Any]) -> None:
                with self._data_lock:
                    ...
                    if status == TaskStatus.WAITING_FOR_FUNDAMENTALS.value:
                        current.status = TaskStatus.WAITING_FOR_FUNDAMENTALS
                    elif status == TaskStatus.PROCESSING.value:
                        current.status = TaskStatus.PROCESSING
                    if payload.get("progress") is not None:
                        current.progress = int(payload["progress"])
                    if payload.get("message") is not None:
                        current.message = str(payload["message"])
                    snapshot = current.copy()
                self._broadcast_event("task_updated", snapshot.to_dict())
                ...
                with self._data_lock:
                    ...
                    if task:
                        if task.status == TaskStatus.CANCELLED:
                            logger.info(f"[TaskQueue] 任务已取消，忽略完成结果: {task_id} ({stock_code})")
                            return None
            ...
            with self._data_lock:
                ...
                if task:
                    if task.status == TaskStatus.CANCELLED:
                        logger.info(f"[TaskQueue] 任务已取消，忽略失败结果: {task_id} ({stock_code})")
                        return None
