...
class SkillManager:
    ...
    def get_skill_instructions(self) -> str:
        ...
        for cat_key in ordered_keys + [k for k in grouped if k not in ordered_keys]:
            ...
            parts.append(f"#### {cat_label} Strategies\n")
            for skill in skills_in_cat:
                rules_ref = ""
                if skill.core_rules:
                    rules_ref = f" (Related core principles: #{', '.join(str(r) for r in skill.core_rules)})"
                parts.append(
                    f"### Strategy {idx}: {skill.display_name}{rules_ref}\n\n"
