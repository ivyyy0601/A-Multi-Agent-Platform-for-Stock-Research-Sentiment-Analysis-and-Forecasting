...
def _handle_get_volume_analysis(stock_code: str, days: int = 30) -> dict:
    ...
    if len(volume) >= 10:
        ...
        vol_trend = "Expanding Volume" if vol_trend_pct > 20 else "Shrinking Volume" if vol_trend_pct < -20 else "Stable Volume"
    else:
        vol_trend_pct = 0
    ...
    if avg_up_vol > avg_down_vol * 1.3:
        pattern = "Healthy Volume-Price (Volume expands on up days, shrinks on down days)"
    elif avg_down_vol > avg_up_vol * 1.3:
        pattern = "Bearish Divergence (Volume expands on down days, shrinks on up days)"
    elif vol_ratio_5d and vol_ratio_5d > 1.5:
        pattern = "Recently Notable Volume Expansion"
    elif vol_ratio_5d and vol_ratio_5d < 0.6:
        pattern = "Recently Notable Volume Contraction"
    else:
...
get_volume_analysis_tool = ToolDefinition(
    ...
    description="Analyse volume-price relationship for a stock. Returns volume ratios, "
                ...
                "and pattern interpretation (healthy/divergence). Useful for confirming trend "
...
def _handle_analyze_pattern(stock_code: str, days: int = 60) -> dict:
    ...
    for i in range(max(0, n - 3), n):
        ...
        if bd < avg_body * 0.1 and (us + ls) > bd * 3:
            patterns_detected.append({
                ...
                "day_offset": -(n - 1 - i),
                "strength": "Weak", "desc": "Bulls and bears balanced, possible trend change signal"
            })
        ...
        if bd > avg_body * 2.5:
            label = "Large Bullish Candle" if is_bullish(i) else "Large Bearish Candle"
            t = "bullish" if is_bullish(i) else "bearish"
            patterns_detected.append({
                "pattern": label, "type": t,
                "day_offset": -(n - 1 - i),
                "strength": "Strong", "desc": "Large body, clear directional momentum"
            })
    ...
    if n >= 3:
        ...
        if (is_bullish(i) and is_bearish(i - 1)
            ...
            patterns_detected.append({
                ...
                "day_offset": -1, "strength": "Strong", "desc": "Bullish candle fully engulfs prior bearish candle"
            })
        elif (is_bearish(i) and is_bullish(i - 1)
              and o[i] > c[i - 1] and c[i] < o[i - 1]):
            patterns_detected.append({
                "pattern": "Bearish Engulfing", "type": "bearish_reversal",
                "day_offset": -1, "strength": "Strong", "desc": "Bearish candle fully engulfs prior bullish candle"
            })
    ...
    if n >= 10:
        ...
        if box_range_pct < 8:
            patterns_detected.append({
                "pattern": "Box Consolidation", "type": "consolidation",
                "day_offset": 0, "strength": "Medium",
                "desc": f"10-day price range {box_range_pct:.1f}%, price oscillating within a band"
            })
    ...
    return {
        ...
        "summary": (
            "No significant patterns detected" if not unique_patterns
