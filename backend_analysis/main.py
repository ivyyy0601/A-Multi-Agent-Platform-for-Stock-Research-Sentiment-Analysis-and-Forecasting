...
def main() -> int:
    ...
    if args.serve_only:
        ...
        # 启动爬虫子进程（news_crawler 和 reddit_crawler）
        import subprocess as _subprocess
        import sys as _sys
        _backend_dir = os.path.dirname(os.path.abspath(__file__))
        _crawler_procs = []
        for _crawler_name in ("news_crawler", "reddit_crawler"):
            _crawler_dir = os.path.join(_backend_dir, _crawler_name)
            if os.path.isdir(_crawler_dir):
                try:
                    _proc = _subprocess.Popen(
                        [_sys.executable, "main.py"],
                        cwd=_crawler_dir,
                    )
                    _crawler_procs.append(_proc)
                    logger.info(f"已启动 {_crawler_name} (pid={_proc.pid})")
                except Exception as _e:
                    logger.warning(f"启动 {_crawler_name} 失败: {_e}")
