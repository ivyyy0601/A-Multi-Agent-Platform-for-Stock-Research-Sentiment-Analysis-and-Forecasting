...
# ---------------------------------------------------------------------------
# External news / reddit feeds (reads from news_crawler / reddit_crawler DBs)
# ---------------------------------------------------------------------------
...
_NEWS_DB = _os.path.join(_BACKEND_DIR, "news_crawler", "news_crawler", "data", "news.db")
_REDDIT_DB = _os.path.join(_BACKEND_DIR, "reddit_crawler", "reddit_crawler", "data", "reddit.db")


def _query_ext_db(db_path: str, sql: str, params: tuple) -> list:
    ...
    try:
        ...
        logger.warning("External DB query failed (%s): %s", db_path, e)
        return []


@router.get("/{stock_code}/ext-news")
def get_ext_news(stock_code: str, days: int = 3, limit: int = 20):
    """从 news_crawler 数据库读取近期新闻。"""
    from datetime import datetime, timezone, timedelta
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).strftime('%Y-%m-%dT%H:%M:%S')
    rows = _query_ext_db(
        _NEWS_DB,
        "SELECT title, summary, url, source, published_at FROM news "
        "WHERE ticker=? AND published_at>=? ORDER BY published_at DESC LIMIT ?",
        (stock_code.upper(), cutoff, limit),
    )
    return {"ticker": stock_code.upper(), "count": len(rows), "items": rows}


@router.get("/{stock_code}/reddit")
def get_reddit_posts(stock_code: str, days: int = 3, limit: int = 20):
    """从 reddit_crawler 数据库读取近期帖子。"""
    from datetime import datetime, timezone, timedelta
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).strftime('%Y-%m-%dT%H:%M:%S')
    rows = _query_ext_db(
        _REDDIT_DB,
        "SELECT title, text, url, subreddit, author, score, num_comments, published_at FROM posts "
        "WHERE ticker=? AND published_at>=? ORDER BY published_at DESC LIMIT ?",
        (stock_code.upper(), cutoff, limit),
    )
    return {"ticker": stock_code.upper(), "count": len(rows), "items": rows}
