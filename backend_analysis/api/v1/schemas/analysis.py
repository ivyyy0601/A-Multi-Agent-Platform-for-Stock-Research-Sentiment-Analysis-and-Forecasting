...
class TaskStatusEnum(str, Enum):
    ...
    WAITING_FOR_FUNDAMENTALS = "waiting_for_fundamentals"
    COMPLETED = "completed"
    FAILED = "failed"
    CANCELLED = "cancelled"
...
class IvyAgentReportResponse(BaseModel):
    """给 TradingAgents/ivy agent 使用的精简同步报告响应。"""

    query_id: str = Field(..., description="分析记录唯一标识")
    stock_code: str = Field(..., description="股票代码")
    stock_name: Optional[str] = Field(None, description="股票名称")
    ivy_report: str = Field(..., description="适合多 agent 消费的 markdown 报告")
    report: Optional[Any] = Field(None, description="完整结构化报告")
    created_at: str = Field(..., description="创建时间")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "ivy_report": "# Ivy Agent Report for AAPL\n\n- Operation Advice: Watch",
                "report": {
                    "summary": {
                        "sentiment_score": 58,
...
class TaskAccepted(BaseModel):
    ...
    status: str = Field(
        ..., 
        description="任务状态",
        pattern="^(pending|processing|waiting_for_fundamentals)$"
    )
...
class TaskStatus(BaseModel):
    ...
    status: str = Field(
        ..., 
        description="任务状态",
        pattern="^(pending|processing|waiting_for_fundamentals|completed|failed|cancelled)$"
    )
